...
if (!empty($_POST)) {
	...
	if ($categoryId > 0) {
		...
		// Je fais le bindValue pour UPDATE
		...
		// Je fais le bindValue pour INSERT
		$pdoStatement->bindValue(':cat_name', $categoryName);// C'est pas besoin de spécifier PDO::PARAM_STR parce que le chaîne de caractères c'est par default
		if($pdoStatement->execute() === false) {
		print_r($pdoStatement->errorInfo());
		}
	}
...
';
	...
	if ($pdoStatement->rowCount() > 0) {
		$allCategoriesList = $pdoStatement->fetchAll();
	}
...
if (isset($_GET['cat_id'])) {
	...
	if ($pdoStatement->execute()) {
		$existentCategoriesList = $pdoStatement->fetch();
		$categoryName = $existentCategoriesList['cat_name'];
	}
}
